...
class TypedSerializer:
    ...
    def serialize_obj(obj):
        ...
        if t == dict:
            for name, value in obj.items():
                result[name] = TypedSerializer.serialize_obj(value)
        elif t == list or t == tuple:
            result[TYPE_FIELD] = type_string
            result[VALUE_FIELD] = list()
            for value in obj:
                result[VALUE_FIELD].append(TypedSerializer.serialize_obj(value))
        elif inspect.isroutine(obj):
            result[TYPE_FIELD] = type_string
            result[VALUE_FIELD] = TypedSerializer.serialize_func(obj)
        elif t == bytes:
            result[TYPE_FIELD] = type_string
            ...
            result[TYPE_FIELD] = type_string
            result[VALUE_FIELD] = TypedSerializer.serialize_instance(obj)
        return result
    ...
    def serialize_func(func):
        ...
        for detail in details:
            ...
            if detail[0] in FUNCTION_ATTRIBUTES:
                result[detail[0]] = TypedSerializer.serialize_obj(detail[1])
                    ...
                    for name in detail[1].__getattribute__(GLOBALS_FIELDS):
                        ...
                        if name in glob:
                            if inspect.ismodule(glob[name]):
                                continue
                            result[GLOBALS][name] = TypedSerializer.serialize_obj(glob[name])
        return result
    ...
    def serialize_instance(inst):
        ...
        for attr in attrs:
            if callable(attr[1]):
                continue
            result[attr[0]] = TypedSerializer.serialize_obj(attr[1])
        return result
    ...
    def deserialize_func(func):
        ...
        for name, o in func[GLOBALS].items():
            glob[name] = TypedSerializer.deserialize_obj(o)
        details.append(glob)
        for attr in FUNCTION_ATTRIBUTES:
            if attr == CODE_FIELD:
                continue
            details.append(TypedSerializer.deserialize_obj(func[attr]))
    ...
    def deserialize_obj(obj):
        ...
        if t == dict:
            ...
            if VALUE_FIELD in obj and TYPE_FIELD in obj:
                return TypedSerializer.deserialize_inst(obj[TYPE_FIELD], obj[VALUE_FIELD])
            for name, o in obj.items():
                tp = type(o)
                if tp == dict:
                    result[name] = TypedSerializer.deserialize_obj(o)
                else:
                    result[name] = o
            return result
        return obj
    ...
    def deserialize_inst(key, value):
        if key == "tuple":
            ...
            return TypedSerializer.deserialize_func(value)
        else:
            return value
